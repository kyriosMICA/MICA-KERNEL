...
app = FastAPI(title="MICA-Kernel API", description="kyriosMICA v3.0 · TQIM-Davoh v3 · Bell/GHZ · MPS · Na⁺/K⁺/Mg²⁺", version="3.1.0", contact={"name":"kyriosMICA","url":"https://kyriosmica.com","email":"direction@kyriosmica.com"})
...
@app.get("/")
async def root():
    return {"api":"MICA-Kernel v3.1","spec":"TQIM-Davoh v3","institution":"kyriosMICA","features":["Bell/GHZ","MPS","Na⁺/K⁺/Mg²⁺ Debye-Hückel","per-bond Z_j","5mC/5hmC/m6A/m4C"],"endpoints":{"POST /analyze":"Full","POST /codon":"Single+MC","POST /mih21":"MIH-21","POST /conditions":"Per-bond","POST /mutability":"Risk map"},"slogan":"Decoding Life. Encoding the Future."}

@app.get("/health")
async def health():
    P = boltzmann_populations(300, 7.4, 0.15); assert abs(sum(P.values())-1)<1e-9
    return {"status":"healthy","engine":"MICA-Kernel v3.1","features":"Bell/GHZ·MPS·Na⁺/K⁺/Mg²⁺"}
