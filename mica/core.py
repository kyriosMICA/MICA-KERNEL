...
def analyze_sequence(sequence, T_K=300.0, pH=7.4,
    ...
    # ── NO filtering, NO ORF detection, NO stop codon break ──
    # Analyze ALL codons exactly as the user provides them.
    # Stop codons (TAA/TAG/TGA) are marked as "*" (stop) in amino_acid
    # but are NOT filtered out — the user decides what to analyze.
    codons = codons_raw
...
def _parse_sequence(raw):
    ...
    for l in lines:
        ...
        seq += l.strip().upper()
    # Replace U with T (RNA→DNA), keep only ATCG
    seq = seq.replace('U', 'T')
